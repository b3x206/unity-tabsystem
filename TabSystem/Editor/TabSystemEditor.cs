    ...
    public override void OnInspectorGUI()
    {
        ...
        if (EditorGUI.EndChangeCheck())
        {
            ...
            foreach (var target in targets)
            {
                ...
                // target value is updated + objects are generated by the 'RegisterCreatedObjectUndo'
            ...
            foreach (var target in targets)
            {
                foreach (var undoRegister in target.TabButtons.Where(tb => !undoRecord.Contains(tb.gameObject)))
                {
